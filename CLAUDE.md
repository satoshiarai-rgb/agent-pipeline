...
| `agents/` | **フェーズの中のチームの定義**（レビューの観点 2 人と、developer 用のリーダー。planner は観点を直接呼び、計画を詰める問いにも観点が答える / A-66・A-68）。Claude Code の plugin として配り、`claude_args` の `--plugin-dir` で読ませる。**配布先の上書きは想定しない**（K-32）。振る舞いをプロンプト側に書き戻すとリレーで薄まる |
