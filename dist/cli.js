...
function eventPaths(dir) {
  ...
  if (!existsSync3(base))
    return [];
  return readdirSync2(base).filter((name) => name.endsWith(".json")).sort().map((name) => join5(base, name));
}
...
function promptCandidates(agent, roots) {
  return [
    join6(roots.repo, ".agent", "prompts", `${agent}.md`),
    join6(roots.central, "prompts", `${agent}.md`)
  ];
}
...
function readConventions(repo) {
  const path = join6(repo, ".agent", "conventions.md");
  if (!existsSync4(path))
    return null;
...
function nextReviewNumber(dir, kind) {
  const reviews = join7(dir, "reviews");
...
function reviewPath(dir, kind, round) {
  return join7(dir, "reviews", `${kind}-${String(round).padStart(2, "0")}.md`);
}
...
function reviewPaths(dir, kind) {
  ...
  return readdirSync3(reviews).filter((n) => n.startsWith(prefix) && n.endsWith(".md")).sort().map((n) => join7(reviews, n));
}
...
var file = (label, rel) => ({
  label,
  find: (dir) => existsSync6(join8(dir, rel)) ? [join8(dir, rel)] : []
});
...
${body}`;
var WRITING_RULES = `**結論を先に書く。** 読み手が最初に要るのは「何を直すか」「何を決めたか」「何が言いたいか」で、
経緯・根拠・検討した過程はその後に置く。前置きを積んでから結論に至る順にしない。

**並列するものは構造で書く。** 規則・観点・選択肢・手順・比較は**箇条書きか表**にし、
...
**区切り文字で並べたものは列挙である。** 中点（・）・読点・スラッシュで 3 つ以上つないだら、
箇条書きか表に開く。**区切り文字が何かは関係ない** — 1 つの文に入っている時点で、そこに
いくつあるかが読み取れない。**括弧で値を添える形（\`優先出資（332,000,000）\`）も、
表にすべきものが文に潰れている合図**である（項目と値の対応は左右 2 列の表にする）。

- **関係・順序・構造は図にする。** 登場人物をまたぐやり取りは \`sequenceDiagram\`、分岐や状態の遷移は
  \`flowchart\` / \`stateDiagram\`、データの関連は \`erDiagram\`、型やモジュールの関係は \`classDiagram\`、
  システムの境界は C4（\`C4Context\` など）。mermaid で書けば GitHub と多くのビューアでそのまま描画される。
  **図に描いたことを文章で書き直さない** — 添えるのは図から読み取れないこと（なぜその形なのか）だけ
- **一直線の手順と、単なる一覧は図にしない。** 番号付きの箇条書きか表で足りる。
  **図のほうが表より長くなるなら表にする。** 図は関係が絡むときだけ元が取れる
- **セルが長くなるからといって表をやめない。** 表のセルには改行も箇条書きも入れられる —
  改行は \`<br>\`、複数項目は \`<ul><li>…</li></ul>\`。**読点や中点でつないで 1 文に潰さない。**
  ただし 1 つのセルに 4 つ以上並ぶなら、列を増やすか行を分けたほうが読める
- **確かめた範囲は 1 行で必ず書く。**「〜の観点と、前回の指摘の修正を確認。問題なし」のように、
  **何を見たか**が分かる 1 行を残す（読み手が「どこまで見た結果か」を判断できるようにするため）。
  **ただし項目ごとの確認結果や、確認作業の経過は書かない** — 1 項目ずつの「問題なし」の列挙は、
  読み手が要る「何を直すか」を埋もれさせる
...
\`phase\` を止まる前のフェーズ（最初なら \`planning\`）に戻して push してください。`
  },
  {
    ...
    title: "中央リポジトリのバージョンが合いません",
    body: () => `進行中の run を壊さないための停止です。**この停止は状態から毎回導かれる**ので、
バージョンが揃った時点で解け、続きから動きます（\`/agent retry\` も要りません）。
