...
function resolveAgent(config, agent) {
  ...
  if (!a)
    throw new Error(`defaults.yml に agents.${agent} がありません`);
...
// src/utils/parse-yaml-document.ts
function parseYamlDocument(text) {
  return $parseDocument(text);
}

// src/utils/set-yaml-fields.ts
function setYamlFields(doc, fields) {
...
function applyStateFile(doc, patch) {
...
function readRunDir(dir) {
  ...
  const records = existsSync2(runsDir) ? readdirSync2(runsDir).filter((n) => n.endsWith(".yml")).sort().map((n) => ({ path: join2(runsDir, n), text: readFileSync(join2(runsDir, n), "utf8") })) : [];
  return { stateText: readFileSync(join2(dir, "state.yml"), "utf8"), records };
}
...
var recordName = (r) => `${r.agent}-${r.run_id}-${r.attempt}.yml`;
var writeState = (dir, file, patch, now) => writeFileSync(join3(dir, "state.yml"), applyStateFile(file.doc, { ...patch, now }));
...
var config = loadConfig(readFileSync2(values.defaults ?? "defaults.yml", "utf8"));
var outcome = () => ({
