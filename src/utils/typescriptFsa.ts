/**
 * typescript-fsa（vendored）
 *
 *   出典: https://github.com/aikoven/typescript-fsa/blob/master/src/index.ts
 *   取得: 2026-09-07
 *   作者: Daniel Lytkin（aikoven）
 *   ライセンス: MIT
 *
 * FSA（Flux Standard Action）の action creator を作る。`actionCreatorFactory(prefix)` の
 * prefix がそのまま ducks の名前空間（`<app>/<reducer>/<TYPE>`）になり、`commonMeta` で
 * 全 action に載る meta（再生の合図など）を指定できる。
 *
 * npm から入れずにここへ写しているのは、上流が更新されない前提の小さな型ユーティリティで、
 * 読めない差分が `dist/cli.js`（コミットする成果物）に入るのを避けるため。
 * **中身は変更していない。** 書式も上流のまま保つ（biome の対象外にしている）。
 * ファイル名だけ、このリポジトリの規則（lowerCamelCase）に合わせている。
 */
