...
/**
 * **全エージェントの成果物に効く書き方の規則。** プロンプトではなくハーネスが差し込むのは、
 * 配布先が `.agent/prompts/<agent>.md` を差し替えても効かせるため（K-15）。ここに置くことで
 * 5 本のプロンプトに同じ規則を複製しなくて済む（複製すると必ず片方が古くなる）。
 *
 * 4 つとも実測を根拠に持つ。構造化は「INVEST の 6 観点が 1 項目に詰め込まれ、その中の規則を
 * planner もレビューチームも拾えず差し戻しが 1 往復増えた」、確認済みの列挙は「レビュー本文の
 * 半分以上が『合っていることを確認した』だった」、中身の引用は「読み手は同じリポジトリを見られる」。
 */
const WRITING_RULES = `**結論を先に書く。** 読み手が最初に要るのは「何を直すか」「何を決めたか」「何が言いたいか」で、
