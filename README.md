...
実装中です。**本物のエージェントで issue から `done`（PR が ready for review）まで実機で通って
います**（1 件あたり 5〜10 実行 / $10 前後）。現時点の制約は次のとおりです。

- 参照は `@v1.0.3` のような**正確なバージョン**（`install.sh` が置くファイルがそうなっています）。自動追随したいなら移動タグ `@v1`。`@main` は開発バージョンです
- Claude の認証はサブスクリプションのトークン（`CLAUDE_CODE_OAUTH_TOKEN`）のみ

## 開発

ランタイムは Node、bun は開発ツールチェーンとして使います。実行時の依存は `redux` 1 本だけで、`dist/cli.js` にバンドルするのでランナー側での install は不要です。
