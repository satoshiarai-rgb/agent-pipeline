...
| `blocked` | 続けられない（成果物が契約を満たさない、レビューが収束しない、実行が失敗した等） | **理由と次の手順が PR にコメントされる。** 原因を直して `/agent retry`（直前のフェーズに戻して再実行） |
...
途中で `blocked` になると、**止まった理由と次の手順が PR にコメントされます**。原因を直して
`/agent retry` とコメントすれば直前のフェーズから再実行されます。上限に達して止まった場合は
やり直しても同じ理由で止まるので、issue を分けて立て直してください。

`manual` の受け入れ条件は人間が確かめます（エージェントに確認できないもの）。この場合も
「何を確かめ、`acceptance.json` のどこをどう直し、そのあと何をするか」がコメントに出ます。
