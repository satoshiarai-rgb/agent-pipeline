...
- [x] A-55: **判断（2026-09-08）。3 件とも決めた。**
  ...
  - (3) **`labels` は上書き不可にした。** `mergeConfig.ts` の `OVERRIDABLE` から外し、`install/config.json` の雛形からも落とした。理由は、起動ラベルが配布先のラッパーの `if:` に直書きなので `trigger` を変えても黙って無視され、`prefix` を変えると起動ラベルが prefix 外になって bootstrap 後に外れなくなること。`docs/installation.md` と `CLAUDE.md` の「上書きできるもの」の一覧からも削除し、`defaults.ts` の該当キーに理由を書いた
