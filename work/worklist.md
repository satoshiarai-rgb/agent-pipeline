...
- [ ] A-53: **reducer/action への移行（K-26 / K-27）。段取りは 1 → 6。1〜4 はタグ `v1`（I-13）と 2 つ目の配布先（R-2）より前に置く**（`state.json` の役割とイベントのファイル形式が変わるため、配布後だと移行が要る）
  ...
  - [x] 7（追加。2026-09-08）: **`validate` コマンドを `finish` に畳んだ。** 検査結果を CLI 引数 5 本（`--result` / `--verdict` / `--api-error-status` / `--detail` / `--acceptance-passed`）で YAML 越しに運び直していたのをやめ、`finish` が `src/commands/validate.ts` を直接呼ぶ。**検査する相手は `start` が記録した in_flight**（`selectInFlightAgent`）で、`--agent` も渡さない
