...
- タグは **`v1.0.3`** まで。**配布先は正確なバージョンを参照する**形（K-30 / K-31。上げるのは `uses:` の
...
- 層: `cli.ts`（引数の解析だけ）/ `redux/`（`runCommand.ts` の分岐、`mapValidationToAction.ts`、
  ...
  `file/`（1 ファイル形式 = 1 モジュール）/ `utils/`（純関数と vendoring）/ `pipelineSettings.ts` / `types.ts`。
...
- 本番経路: `agent-dispatch.yml` の `run` job が `start` → `compose` → `base-action@v1.0.215` → `finish`
...
- **精査（2026-09-08）**: 死んだコード・導出値・二重定義を落とした（`approve.yml` は誰からも
  ...
  **`agent-comment.yml` の実害あるバグを 1 件修正**（`/agent retry` の成功後に「受け付けませんでした」を
...
- I-12: **`install/` 一式を作った。** 配布先に置くファイルの原本（`agent.yml` / `conventions.md` /
  ...
  **`install/install.sh` で 1 コマンドで置ける**ようにした（既存は上書きせず、置いたあとの手順を
  ...
  YAML の正を `install/agent-pipeline.yml` に決めた**（A-51。`work/verify/check-agent-dispatch.yml` は削除し、
...
| K-30 | **配布先は正確なバージョン（`@v1.0.3` のようなパッチまで含むタグ）を参照する。タグの中の中央の自己参照も同じバージョンにする**（2026-09-10 決定。K-29 の「移動タグ `@v1` を参照する」を置き換え） | 移動タグだと、パッチを入れた瞬間に配布先の挙動が変わる。**同じ入力で同じ動きが再現できること**を優先し、上げるときは配布先の `uses:` を書き換えて PR にする（4 行）。`v1` は「最新の v1 系」を指す目印として残すが、既定では参照しない。`scripts/release.sh` がリリースごとに (1) main の `install/agent-pipeline.yml` の参照をそのバージョンにして push、(2) タグの中の中央の自己参照（composite action・reusable workflow・中央の checkout の `ref:`）もそのバージョンに書き換える。**(2) が無いと、配布先が固定してもハーネス本体（`dist/cli.js`）が別のバージョンから来る**（A-11）。テストは「配布先は `v<major>.<minor>.<patch>`、中央は `main`」を固定する |
| K-31 | **バージョンを上げるのは配布先の作業（`uses:` の 4 行）。自動化は Dependabot に任せる**（2026-09-10 決定） | 正確なバージョンを参照する（K-30）ので、上げないと修正が届かない。中央から配布先へ PR を出すボットは作らない（**既存の仕組みに乗る方が保守が要らない**）。`install/dependabot.yml` を配布物に足し、`github-actions` の ecosystem を置く（既に `.github/dependabot.yml` があるリポジトリでは `install.sh` が skip するので、項目を自分で足す旨を案内に書いた）。**`install.sh --force` は上げる手段ではない**（`.agent/` の雛形まで戻る）。`pipeline_version` が上がるバージョンに切り替えるときは、**進行中の run が無いタイミングで上げる**（噛み合わないと `pipeline_version_mismatch` で止まる。止まった issue は立て直す / K-14） |
| K-34 | **planner は計画を詰める決定を自分で採らない。問いは planner が立て（推奨答つき）、答えはレビューチームが出す。grilling の 2 役（`grilling-planner` / `grilling-answerer`）は廃止**（2026-09-24 決定） | grilling の 2 役は Opus のサブエージェントで、レビューチーム（Sonnet）とは別に毎ラウンド起きていた。**問いを立てる役は親が兼ねても読み込みがほとんど増えない** — 親は問いを立てる前に issue と既存コードを読み終えており、`grilling-planner` に渡していたのは「そのラウンドで何を扱うか」の要約だった。**答える役だけは親に兼ねさせない** — 問いを立てた本人が裁くと、前提の偏りが誰にも見られないまま計画に入る。そこでチームに答えさせる。事実（ファイルを読めば分かること）は親が調べ、決定だけを問いにする。答えが「未確認」か「割れた」（観点どうしで不一致）なら、親は推奨で埋めず `status: open` で人間に回す。**相談と grilling は 1 本にした**（問いのラウンド 3 回 + 完了時の点検 1 回。リーダーの呼び出しは従来と同じ最大 4 回） |
| K-33 | **判断の記録は `journal/` に書かせ、`reversibility: hard` だけをハーネスが `decision-records/` へ寄せる**（2026-09-17 決定。`pipeline_version` 2 → 3） | 実測で planner 1 ラウンドが 8 件の記録を書き、`page-title-from-ja-yml` のような粒度のものが並んだ。**ADR は「後戻りが困難な判断」の記録という語なので、この粒度を全部 `decision-records/` に置くと名前が嘘になり、人間が承認時に見るべきものが埋もれる。** 分け方は 2 つの原則で決めた: (1) **エージェントの書き込み先は 1 箇所（`journal/`）に固定する** — 置き場を選ばせると `reversibility` の判断と場所の判断が二重になる。ハーネスが `finish` のたびに `reversibility` から導いて寄せ直すので、後のラウンドで `hard` に変えればその時点で移る（`routeJournal`。規則は `src/file/journal.ts` の `DIRS` 表 1 つ） (2) **`conversations/` は統合しない** — ディレクトリ境界が「下流のプロンプトに入るか」を表しており（`compose.ts` の `DECISIONS` は `inputs`、`conversations` は書き込み先を渡すフラグ）、統合すると会話が下流に流れ込んで高くつくか、frontmatter で選り分ける処理が要る（同じ分割を作り直すだけ）。**ADR 相当でも run ディレクトリの外には出さない** — あくまで ADR の「候補」で、昇格させるかは人間の判断 |
| K-32 | **フェーズの中のチーム（レビューチーム。grilling の 2 役は K-34 で廃止）は agent 定義として中央が配り、配布先の上書きは想定しない**（2026-09-16 決定） | 振る舞いの指示を親のプロンプトからバケツリレーさせると薄まる（実測: 「リーダーには要約させる」が届かず、やり取りの記録が 7.6KB → 19.6KB に増えた）。定義ファイルに直接書けばリレーが消え、親が渡すのは「何を見てほしいか」だけになる。**上書きの経路は増やさない** — フェーズのプロンプト（`.agent/prompts/<agent>.md`）は配布先が差し替えられるが、チームの定義は中央のものを使う。`.claude/agents/` に置けば配布先が上書きできてしまうが、`.claude/**` は人間しか置けない領域（K-19）で、差し替えの入口が 2 系統に割れるため採らない |
| K-29 | **タグは 2 本（動かない `v1.0.0` と移動する `v1`）。タグの中では中央の自己参照を `@v1` に書き換える**（2026-09-09 決定） | 配布先が参照するのは移動タグ `@v1`（Q-3）。**そのままタグを打つと、reusable workflow は `@v1` から来るのにハーネス本体（`dist/cli.js` を持つ composite action）は `@main` から来るため、ピンが効かない**（A-11 のバージョンずれ）。そこで `scripts/release.sh` が「中央の自己参照を `@<major>` に書き換えたコミット」を作り、そこにタグを置く。**main は `@main` のまま**なので、検証用リポジトリ（`@main` を参照）は常に最新を通せる。タグの中だけが自己完結する。あわせて `scripts/__tests__/workflows.test.ts` が「配布先のラッパーは `@v1`、中央は `@main`」の 2 種類しか無いことを固定する。**タグ `v1` = `pipeline_version` 2**（イベントログに移した時点で 2 に上げた / A-53 段取り 2）。`pipeline_version` を上げる変更は major タグも上げる |
| K-28 | **成果物の形式は JSON Schema にしない。検証は `src/file/*.ts` の手書きの検査 1 箇所で、仕様の正は `docs/agent-contract.md` の表**（2026-09-08 決定） | 理由は 3 つ。(1) **検証のエラーメッセージが本体**で、`acceptanceProblems()` が返す日本語がそのまま `blocked_reason` に載り、`explain` の案内・`docs/troubleshooting.md` の表・プロンプトが依存している。JSON Schema の汎用エラー（`/criteria/2: must have required property 'command'`）を日本語に写し戻す表を別に持つことになり、コードは増える。(2) **依存が増える**（実行時依存は `redux` 1 本という方針。自前で schema を解釈するなら今の 20 行の検査より大きくなる）。(3) 規則が条件付き（`verification: automated` なら `command` 必須、`status: passed` なら `evidence` 必須）で、`if/then/allOf` は「上から順に見て理由を返す」形より読みにくく、契約の表との 1 対 1 対応も崩れる。**判断が変わる条件**: 人間のエディタ補完が要るようになったとき（`$schema` を検証の正とは別に置く。ただし一致を確かめるテストにバリデータが必要）か、TypeScript 以外の消費者が現れたとき。`reviews/*.md` の frontmatter と決定記録も同じ理由で、正規表現と 1 行パーサ（`src/utils/frontmatter.ts`）に留める |
| K-26 | **状態の変更を Redux の作法に則った `reducer(state, action) → state` に畳む。reducer は副作用を知らない**（2026-09-07 決定） | 判断の入口が 5 本（`finish` / `approve` / `request-changes` / `retry` / `block`）に分かれ、拒否理由の語彙が 3 ファイル（`retry.ts` 5 種 / `human-transition.ts` 2 種 / `transitions.ts` 1 種）、`blocked_reason` の生成が 3 ファイル（`finish.ts` 7 種 / `route.ts` 2 種 / `block.ts`）に散っていた。**正は追記専用のイベントログに移り、`state.json` はその射影（人が読む確認用スナップショット。編集しても効かない）になる。** 部品: (1) **action は FSA**。最上位は `type` / `payload` / `error` の 3 キーで、`type` は **ducks の MUST に従って名前空間付き**（`<app>/<reducer>/<TYPE>` = `agent-pipeline/run/REVIEW`。定数は UPPER_SNAKE で `types.ts` に置く）。この文字列は全イベントファイルに永続化される。**イベントのファイル名には type の末尾セグメントを小文字にしたものを使う**（`/` はファイル名に使えないため）。`at` / `by` / `run_id` / `attempt` は **payload** に置く（記録される事実なので）。失敗系は `error: true` + `payload.reason`（FSA は payload に Error を SHOULD とするが、`JSON.stringify(new Error())` が `{}` になり追記専用ログに残せないので外す） (2) **ガードは `next` を呼ばない middleware**。拒否理由は `dispatch` の戻り値（thunk と同じ仕組み） (3) **reducer は表を引くだけの純関数**。遷移の辺は `defaults.transitions` のまま、事象は `events.ts`、カウントは `counts.ts`（action type ごとに `total_steps` / `rounds` を +1 するかの表。人間の差し戻しは +0 なので A-41 が表として読める） (4) **副作用の分界**: **ファイル入出力は middleware**（`hydrate` 読み / `event-log` 追記 / `snapshot` 書き出し）、**ファイルに触らない射影は `subscribe`**（`label` / `chain` / `comment`。`outputs` に積み、実行主体は `agent-dispatch.yml` のまま） (5) **読み取りも action**。`dispatch(init())` を最初に行い、`hydrate` middleware が `events/*.json` を名前順に読んで**1 件ずつ `meta: { hydrate: true }` を付けて dispatch** する。`meta` は「その dispatch の文脈」なので**永続化しない**（ログに書くのは `type` / `payload` / `error`）。**各 middleware が自分で `meta.hydrate` を見て分岐する**（合成で外さない。見通しのため）。**とくにガードは再生中に素通しする — 過去は検証しない**。現在の設定で過去を再検証すると、`.agent/config.json` で上限や `approvers` を変えた後に過去のイベントが弾かれ、fold が実際と違う phase を返す（A-19 との相互作用） (6) 再生が終わったら `hydrated` action で **state にフラグを立て**、subscriber はそれを見て書き込みを判断する（subscriber は引数を受け取らず `meta` を見られないため）。`hydrated` はログに追記せず、スナップショットにも出さない（ログから再現できない値をファイルに出さない） (7) **`redux` をバージョン固定で依存に入れる**（実測: `createStore` + `applyMiddleware` + `combineReducers` + `compose` を bundle して 13.3KB、`dist/cli.js` は 44.7KB → 約 58KB。gzip 換算 2〜3KB）。`legacy_createStore` を使う（Redux 5 で `createStore` は非推奨）。**RTK は入れない**（immer / reselect を連れてきて桁が変わる）。**`combineReducers` は使わない**（`review` が `phase` と `rounds` を同時に動かし `*_exceeded` が両方を見るので、スライスに割れない）。**入れないもの**: ストア以外の Redux 周辺（`subscribe` は使うがミドルウェア以外の拡張は無し）、dispatch ループ、Rx。**1 起動 1 action、副作用の実行主体は `agent-dispatch.yml`** という構造は変えない (8) **フォルダは ducks**（`src/store/run/`。reducer を default export、creators / selectors / types を named export）。**action の型定義と creator は別ファイル**で 1:1（`actions/<type>.ts` と `creators/<type>.ts`） (9) **イベントは 1 ファイル 1 イベントで `events/` に置く**（run ディレクトリ直下には置かない — 人が読む成果物が埋まるため）。名前は `<at>-<run_id>-<attempt>-<type の末尾セグメントを小文字にしたもの>.json` で、**`at` は ISO 基本形式**（`20260907T054512Z`。コロンを含めると Windows でチェックアウトできない）。先頭が時刻なので名前順が畳み込みの順序になり（いまの `runs/` は名前順がエージェント名順で時系列にならない）、`run_id` と `attempt` を挟むので並行 push でも名前が衝突しない（B-1 の実測: ファイルを分ければマージは常に両方保持）。**1 本の jsonl にはしない** — 追記が同じ行域に集まり、rebase で競合するか静かに順序が入れ替わる（K-18 から K-25 への変更と同じ理由） (10) **副産物**: `deriveRunStats` と `runRecord.ts` が消える。A-32（唯一の silent corruption 経路。`state.json` の並行更新が rebase で自動マージされる）が**構造的に不要**になる（追記専用 + 導出。スナップショットが混ざっても次の fold が上書きする）。A-34（`log.md`）はイベントの射影になる (11) **スライスは 2 つで `combineReducers` を使う。**`store/info`（**設置**: `issue` / `branch` / `pipeline_version` は bootstrap イベント由来で永続、`dir` / `run_id` / `attempt` / `dry_run` / `hydrated` は起動時に `CONFIGURE` で注入する非永続）と `store/app`（**実行状況** = ログの畳み込みそのもの: `phase` / `blocked_reason` / `blocked_from` / `counts` / `in_flight`）。スナップショット（`state.json`）は `app` 全体 + `info` の永続部分だけを書く。**それ以上は割れない** — `app` の中で `phase` と `counts` を別スライスにすると、`phase` がラウンド上限の判定で `counts` を読み、`counts` が「どの phase の action か」で加算先を決めるので相互参照になる。`app` の中はフィールドごとの関数（引数は `app` 全体、戻り値は 1 フィールド）に分ける。**`Config`（`.agent/config.json` を重ねたもの）は state に入れない** — `combineReducers` 越しに `app` の reducer が `info` を読めないので、どうせ `createAppReducer(config)` のクロージャ注入が要る。両方に置くと二重化する。スライス名を `config` にしないのも同じ理由（`state.config.issue` と `config.limits` が並ぶと読み手が混乱する） (12) **`in_flight` は導出をやめて `app` のフィールドにする**（`agent_started` で立て、終了系の action で落とす）。イベントの走査が消え、`route` は state を見るだけになる (13) **`src/file/` は残る**（1 ファイル形式 = 1 モジュール）。呼び出し元が変わるだけで、`event-log`（新規）と `state-file` は middleware から、`review-file` は middleware（人間の差し戻しの副作用）と `compose` から、`acceptance-file` / `decision-records` / `execution-log` / `prompt-file` はコマンド（`validate` / `compose` / `explain`）から呼ぶ。**例外は `config-file` だけで、store を組み立てる前に読む** — `createAppReducer(config)` が store 生成時点で `limits` を必要とし、middleware は dispatch が始まってからしか走らないため (14) **selector を置く**（`reselect` は入れない。1 起動 1 dispatch でメモ化する対象が無い。引数は root state = ducks の慣習）。理由は**同じ導出を subscriber とコマンドの両方が使う**こと: `selectLabel` は `label` subscriber と `label` コマンド、`selectBlocked` は `comment` subscriber と `explain` コマンド、`selectContinueChain` は `chain` subscriber と finish 相当の出力、`selectNextAction` は `route`、`selectSnapshot` は `snapshot` middleware。あわせて **`comment` subscriber は本文を作らない** — 「コメントが必要」という output だけを出し、markdown は `explain` コマンドが `acceptance.json` を読んで組み立てる（今と同じ 2 ステップ）。これで「subscriber はファイルに触らない」が例外なく成立する (15) **配置（2026-09-07 に確定）**: redux 関連はすべて `src/redux/` 以下。**状態と判断は `store/` の中**（`store/createStore.ts` が RootState と組み立て・初期化 / `store/guards.ts` / `store/selectors.ts` / `store/global/`（スライスを跨ぐ action = `init` / `RESTORE`。reducer を持たない）/ `store/app/` / `store/info/` / `store/middlewares/`）、**外との境界だけ `redux/` 直下**（`index.ts` 公開 IF / `runCommand.ts` CLI の分岐 / `mapValidationToAction.ts` 検証結果 → action の写像 / `explain.ts` 状態 → markdown の射影）。**実行の判断に store の値を使うものは `redux/` に置き、使わないものだけ `commands/` に残す**（2026-09-08）。`explain` は `selectStatus` を読み、`validate` は検査する相手を `selectInFlightAgent` から受け取るので `redux/` へ移した（`redux/explain.ts` / `redux/validate.ts`）。`commands/` に残るのは `compose` だけ。**ducks の 1 スライスは `actions.ts` / `reducer.ts` / `index.ts` の 3 ファイルだけで、それ以外は置かない** — 状態・遷移表の引き方・retry の戻り先・カウント表・遷移のルール表・フィールドの合成はすべて `reducer.ts` にまとめ、スライスを跨いで読むもの（selector・ガード）は `src/redux/` 直下に出す。**CLI の語彙 → store 操作は `redux/runCommand.ts` の分岐 1 つ**（store を使わない 2 / 読むだけの 3 / snapshot / 状態を変える 6 の順に `if` を並べる）で、1 行になったコマンドを 9 ファイルに分けない。**表（`Record<string, Command>`）・ケースごとの型の union・`is` の type guard・引数を組み立てるヘルパは置かない** — 2026-09-08 のレビューで 3 段（表 → 型 → ヘルパ）を全部畳んだ。1 コマンドの処理（引数を読む → action を作る → dispatch → 出力を射影する）が 1 つの `if` ブロックに並ぶ形が正。共通の前処理（`createStore`）はブロックの手前に 1 回だけ書き、`isRejection` の判定は**ガードのある人間起点の 3 つだけ**に置く（`GUARDS` に無い action の dispatch は拒否を返さないため） (16) **FSA と reducer builder は npm から入れず `src/utils/` に vendoring する**（`typescript-fsa`（aikoven, MIT）と `typescript-fsa-reducers`（dphilipson, MIT）。ファイル名は上流のまま、ヘッダに出典 URL・作者・ライセンス・変更点を書き、書式を保つため biome の対象外にする）。`actionCreatorFactory(prefix)` の prefix がそのまま ducks の名前空間になり、`commonMeta` が `meta.hydrate` になる。読めない差分がコミットする `dist/cli.js` に入るのを避けるため、依存は `redux` 1 本に留める — 段取りは A-53 |
...
| K-23 | ~~**`blocked` からの復旧は `/agent retry`。戻る先は履歴（`runs/` の最新レコードの `phase`）が決める**~~（**K-27 で置き換え**。`/agent retry` というコマンドと認可の規則は変わらず、戻り先の決め方だけが「履歴の走査」から「fold の `blocked_from` + 決め打ち表」に変わる） | 手で `state.json` を書き換えるのが唯一の復旧経路だった（実機 #11 でも `completing` に戻す操作を手でやった）。遷移表は前向きの辺しか持たないので逆辺を足すのではなく、**どのフェーズが走っていたかを既に持っている実行レコード**から決める（新しい表を足さない）。受け付けないのは (1) `blocked` 以外の phase (2) `*_exceeded` で止まったもの（やり直しても同じ理由で止まる。issue を分けて立て直す方が正しい）(3) レコードが無いもの (4) 直前のレコードが閉じていないもの（`route` が in_flight と見て動かさないので、戻しても静かに止まったままになる）。**戻り先は構造的にエージェントのフェーズだけ**になる（レコードは `startRun` が作るので、人間を待つ `awaiting_human` / `bootstrap` / `done` のレコードは存在しない）。認可は approve と同じ `approvers`。`pipeline_version_mismatch` は走る前のフェーズが state から失われているため手で直す 1 ケースとして残る — `src/commands/retry.ts`、`agent-comment.yml`、K-13 |
| K-24 | **`blocked` になったら、理由と次の手順を PR にコメントする**（設計書 §5.5 の未実装部分） | 文面は理由から組み立てる表を `src/redux/explain.ts` に置き（`blocked_reason` の文字列と 1 対 1）、ワークフローは貼るだけにする。`acceptance_not_passed` のときは**未達の受け入れ条件を表にして**、`manual` の項目の手順（確かめる → `status`/`evidence` を直す → push → `/agent retry`）を出す。実装を直す必要がある場合は retry では戻れないので `phase` を `developing` にする、まで書く。`*_exceeded` では retry を案内せず「issue を分けて立て直す」に導く。コメントは `GITHUB_TOKEN` で行う（K-22） — `src/redux/explain.ts`、`agent-dispatch.yml` |
| K-22 | **ラベルの射影と PR へのコメントは `GITHUB_TOKEN` で行う。App トークンは連鎖させたい push だけに使う** | App トークンで issue のラベルを張り替えると `issues.labeled` が発火し、配布先のラッパー（`issues` / `issue_comment` を購読している）が**全ジョブ skipped の空の workflow run** を毎フェーズ 1 本ずつ作る。実機の compass-wiki issue #11 では `issues` イベントの run 27 本中 25 本が空だった。`GITHUB_TOKEN` が起こしたイベントは新しい run を作らない（`workflow_dispatch` / `repository_dispatch` を除く）ので、**連鎖に App トークンが要る**（K-1 の裏返し）のと同じ理由で、**射影には `GITHUB_TOKEN` が要る**。中央の reusable workflow 側で `permissions` に `issues: write` / `pull-requests: write` を宣言する。残る空の run は人間が `agent:go` 以外のラベルを付けたときと `/agent` 以外のコメントを書いたときだけで、イベント側に本文・ラベル名のフィルタが無いため防げない — `agent-dispatch.yml` / `agent-bootstrap.yml` / `agent-comment.yml`、`scripts/project-labels.sh` |
| K-21 | **規模超過は停止条件にしない。PR に警告を出して作業は進める** | 1 PR あたり 5〜10 ファイルという上限は**目安**であって、パイプラインを止める条件ではない。planner が超過と判断したら `plan.md` に分割案を添えて計画を完成させ、ハーネスは PR に警告コメント（根拠のパス、分割するなら PR を閉じて issue を分ける、そのまま進めるなら何もしない）と `::warning::` を残して先へ進む。分割するかは人間が PR を見て決める。設計書 §1 の「実装に進まず停止する」から変更（A-49） — `src/commands/finish.ts`、`agent-dispatch.yml`、契約 §4 |
...
- [x] A-53: **完了（2026-09-08）。reducer/action への移行（K-26 / K-27）。段取り 1〜7 すべて。** 状態の正は追記専用のイベントログ、`state.json` はその射影、状態の変更は Redux の store（ducks 2 スライス + middleware 5 本 + selector）を通る。実機（`compass-wiki` issue #15 / #17）で dry-run を 2 回通した
  ...
  - [x] 2: **完了（2026-09-08）。イベントログが状態の正になった。** `events/<連番>-<timestamp>-<run_id>-<attempt>-<type>.json`（`src/file/eventLog.ts`）。**先頭の連番が畳み込みの順序を決める** — 時計の分解能や `run_id` の桁数に依存しない（`9999999999` と `17301992044` を文字列で並べると桁数の少ない方が後になる。決定記録が踏んだのと同じ問題 / K-25）。書くのは `type` / `payload` / `error` だけで `meta` は落とす。`hydrate` middleware が `init` で全イベントを 1 件ずつ `meta.hydrate` 付きで再生し、`eventLog` middleware が追記する（`APPENDABLE` の表）。`BOOTSTRAP` は**スライスを跨ぐ action**（`info` が識別子、`app` が phase を `planning` に）なので `store/global/` に置いた
    ...
    - **`pipeline_version` を 2 に上げた**（バージョン 1 の run は `events/` を持たないので、続けると状態を失う / K-13）
  ...
  - [x] 7（追加。2026-09-08）: **`validate` コマンドを `finish` に畳んだ。** 検査結果を CLI 引数 5 本（`--result` / `--verdict` / `--api-error-status` / `--detail` / `--acceptance-passed`）で YAML 越しに運び直していたのをやめ、`finish` が `src/redux/validate.ts` を直接呼ぶ。**検査する相手は `start` が記録した in_flight**（`selectInFlightAgent`）で、`--agent` も渡さない
    ...
    - **この dry run で実機バグ 2 件が出た**（どちらも修正・push 済み）:
      1. **配布先のラッパーが無効化されていた。** 削除した `approve.yml` を検証用の `check-agent-dispatch.yml` がまだ参照していて、**ワークフローファイル自体が invalid**（どのイベントでも 0 秒で失敗）。承認は `/agent approve` のコメント経路だけなので `approve-manual` ジョブを削除した。**`install/agent-pipeline.yml`（配布の正）には参照が無いので配布先は無事**
      2. **`bootstrap` に `--issue` が届いていなかった。** `agent-bootstrap.yml` は `with: issue:` を渡していたが `action.yml` に入力の宣言が無く、**composite action は未宣言の `with:` を黙って捨てる**ため `--issue が必要です` で exit 2。入力（`issue` / `branch`）を宣言し `run-cli.sh` から転送した。あわせて `scripts/__tests__/workflows.test.ts` に **drift を捕まえるテスト 2 件**（`with:` のキーが `action.yml` にあるか / 宣言した入力が `run-cli.sh` を通るか）を足した — 段取り 2 で heredoc を CLI に置き換えたときの取りこぼしで、テストが無ければ次も同じ形で漏れる
...
- [x] A-54: **完了（2026-09-08）。文書の食い違いを直した。** サブエージェントで `docs/` 4 本・`install/` 全ファイル・`README.md` を実装に突き合わせ、**利用者が実際に踏むもの**から順に直した。
  ...
  - その他: 存在しない `transitions` キーへの言及、`npm 依存はゼロ`（`redux` 1 本をバンドルしている）、`id-token: write` の用途（現時点では未使用）、`agent-bootstrap.yml` の draft PR 本文の `runs/`、`action.yml` のコマンド一覧に `bootstrap`、`install/README.md` の `mkdir -p` 欠け、`/agent retry` の説明（「直前のフェーズ」→「同じフェーズ」）

- [x] A-55: **判断（2026-09-08）。3 件とも決めた。**
  - (1) **到達しない分岐は消さない。** `middlewares/reviewFile.ts` の `kind = "dev"`（将来 dev_review でも人間が差し戻せるようにするときの余地）と `store/global/selectors.ts` の `no_transition_for_phase`（フェーズを足したときに黙って進まない状態を作らないための番犬）。**どちらもコメントで「今は到達しないが残す理由」を書いた** — 次の掃除で消されないため
...
- [x] I-13: **完了（2026-09-09）。タグ `v1.0.0` と `v1` を打った**（どちらもコミット `21e69b9` を指す）。手順は `scripts/release.sh` に入れた（K-29）。
  - **タグの中では中央の自己参照が `@v1`**（composite action・reusable workflow・中央の checkout の `ref:`）。配布先が `@v1` にピンすると、workflow とハーネス本体（`dist/cli.js`）が同じバージョンから来る（A-11 のバージョンずれをここで解消した）
  - **main は `@main` のまま**。検証用リポジトリ（`compass-wiki`）は `@main` を参照しているので、常に最新を通す
  - 配布物 `install/agent-pipeline.yml` は `@v1` を指す。`docs/installation.md` / `README.md` も「`@v1` を使う。固定したいなら `@v1.0.0`。`@main` は開発バージョン」に更新
...
- [x] A-48: **完了（2026-09-09）。`.claude/**` の扱いをプロンプトで 2 点直した。**
  - (1) **planner に理由まで書かせる。** 「`.claude/**` は書けない」だけでは、**成果物しか渡らないレビュアーには自明でない**（追跡済みのファイルがそこにあるリポジトリでは「なぜ書けないのか」が読み取れず差し戻される。実機 #11 で起きた）。**Claude Code が sensitive file として拒否し、許可ルールでも開けられない**という事実まで「前提」に書かせる
  - (2) **成果物は run ディレクトリの `staged/` に置き、設置（移動）は人間に依頼する**（2026-09-09 の判断。リポジトリ直下に置く案は採らない）。`staged/README.md` に「人間への依頼」として (a) どのファイルをどこへ移すか（そのまま貼れるコマンドの形で） (b) 移したあと何を確認すれば設置できたと言えるか (c) **移さなければ何が効かないか** を書かせる。run ディレクトリは issue ごとに閉じるので、依頼は具体的に書くことも明記
  - 完了報告（`completion.md`）の「人間に確認してほしいこと」に **`staged/` があれば必ず挙げる**ようにし、`done` の PR コメントにも `staged/README.md` のリンクが並ぶようにした（テストで固定 / A-57）
...
- [x] Q-3: **決定（2026-09-10。K-30）。許容しない — 配布先は正確なバージョン（`@v1.0.3`）を参照する。** 移動タグだとパッチを入れた瞬間に配布先の挙動が変わり、進行中の run の途中でプロンプトが入れ替わる。バージョンを上げるのは配布先の作業（`uses:` の 4 行）で、自動化は Dependabot に任せる（K-31）
...
- [ ] Q-6: 入力トークンの固定オーバーヘッドをどこまで削るか。V-13 の実測では 1 実行あたり約 17k 入力トークン（`cache_creation` 6056 + `cache_read` 10591）で、**自前のプロンプトは 2 トークン**だった。つまり削減対象はシステムプロンプト preset・ツール定義・skills 一覧であって、プロンプト文の圧縮ではない。手段は V-15（ツール削減）と V-17（preset / skills）。加えて、実運用のコストは固定分より**ターン数 × 再送される履歴**が支配的（developer は `max_turns: 300`）なので、`max_turns` とプロンプトの範囲の方が効く
- [ ] Q-8: **チームの費用対効果を決める（残す / 絞る / やめる）。** `e330a63`（planner と developer にレビューチーム）と `b5d23d1`（チームを agent 定義として plugin 化）で入れたチームが、**費用を一桁変えている。** 2026-09-17 に compass-wiki の worktree で issue #101（画面ページ 1 枚の追加）をローカル実行した実測では、**計画フェーズだけで約 $119**（planner 3 ラウンド + plan-reviewer 2 ラウンド、4 ラウンド目の planner の途中で中断）。CI 実機 #11 は**全 5 フェーズ完走で $10.37** だったので、同じ規模の仕事で 10 倍以上になっている。#11 はチームを入れる前の計測なので、差はほぼ丸ごとチームの追加分。
  - **内訳**（`~/.claude/projects/<配布先>/*.jsonl` の `message.usage` を合計。Opus 5 の単価は入力 $5 / 出力 $25 / cache write $6.25 / cache read $0.50 per MTok）: 本体 5 セッションが出力 489K・cache write 4.24M・cache read 64.5M で **$71.0**、サブエージェント 18 本が出力 275K・cache write 3.82M・cache read 34.2M で **$47.9**
  - **支配的なのは cache read 98.7M トークン（全体の約 4 割・$49）。** サブエージェントは 18 本で、**ラウンドごとに 6 / 10 / 2 本**（3 本目は中断）。**差し戻しを受けて増えており、収束しないまま投入量が増える形になっている。** Q-6 は固定オーバーヘッドの話だったが、実際に効くのは**サブエージェントの本数 × それぞれが積む履歴**である
  - **重複の内訳（実測）。** 各ラウンドで複数のサブエージェントが触ったパスを数えると、のべ 130→実体 55（2.4 重）/ のべ 221→実体 66（**3.3 重**）/ のべ 36→実体 23（1.6 重）。ただし**全部が無駄ではない**。(1) `plan.md` / `acceptance.json` を 10 本中 9 本が読むのは設計どおり（点検対象） (2) `lenders_controller.rb` / `ja.yml` / `partner-index.md` を 7 本が読むのは**独立していることに価値がある**（planner の `lenderCandidates` 誤読を 3 人が独立に指摘して訂正させた） (3) **`bin/validate` を 7 本、`bin/new-page` を 4 本が読むのは純粋な重複**。ハーネスのスクリプトが何を検査するかは解釈の余地が無く、何回読んでも同じ答えにしかならない。**線引きは「解釈が割れうる実装の振る舞い」と「決まりきった事実」の間**にあり、いまは両方を等しく全員に読ませている。役割分担そのものは効いている（観点は「既存実装との一貫性」「受け入れ条件の漏れ」「スコープと壊れる箇所」でプロンプト段階から分かれている）
  - **チームは仕事はしている。** レビューチームは planner が落としていた「金融機関の画面に `require_permission` が無い」を拾い、2 ラウンド目には planner 自身の誤り（`lenderCandidates` を一覧のセルが参照していると誤読したこと）を 3 人が独立に指摘して訂正させた。一方で **plan-reviewer が 2 ラウンド連続で `request_changes`** を返しており（どちらも「`sources` に無いファイルを根拠にしている」という同じ形）、収束していない。planner 1 ラウンドは約 29 分
  - **打った手（2026-09-17）**: (1) レビューチームの 4 本を `claude-sonnet-5` にした（grilling とフェーズのレビュアーは Opus のまま） (2) **観点を 3 つから 2 つに統合した** — `reviewer-functional`（既存実装との整合 + 要件の達成）と `reviewer-nonfunctional`（実行可能性 + 性能 + セキュリティ）。統合の根拠は、ラウンド 2 で 3 人が独立に同じ指摘を返したことと、**「性能・セキュリティ」が文書だけの配布先では定義として成立しておらず**、planner のプロンプトが毎回「観点を読み替えて」と指示し直していたこと。非機能側に「実行可能性」（受け入れ条件のコマンドが実際に走るか、触ってはいけない領域、生成物の同期）を明記して、読み替えを不要にした
  - 残る候補: (c) grilling とレビューを 1 ラウンドに固定する (d) サブエージェントに `repos/` を読ませず、親が読んだ結果だけを渡す — **ただし (d) は隔離をやめる形でもある。** サブエージェントが読んだ 25 ファイルは短い履歴の中で 1 回課金されて消えるが、同じ読み込みを親が行うと親の履歴に残り以降のターンすべてで再送される。**cache read が支配項である以上、統合すると高くなる可能性がある** (e) チームは planner だけに残し developer からは外す。**どれも「レビューが拾っていた穴を拾えなくなるか」を測ってから決める**
  - **打った手の結果（2026-09-17、compass-wiki issue #104 と #105 で実測）。効いてはいるが、費用の主因ではなかった。**
    - **定義側の `model:` は親の `--model` に勝つ（解決）。** V-19 で確認できていたのは**ツール**制限で親が勝つことだけで、モデルは未測定だった。#104 のサブ 10 本は `reviewer-*` だけが `claude-sonnet-5`、grilling は `claude-opus-5` のままで、狙いどおり振り分いた（判別は `~/.claude/projects/<配布先>/*/subagents/*.jsonl` の `message.model`。変更前は 797 メッセージすべて `claude-opus-5`）
    - **リーダーが待たずに返す不具合を見つけて直した。** #104 でリーダーが観点 2 本の完了前（40 秒）に返り、planner が空の結果を受け取って同じ点検をやり直していた（リーダー + 観点 2 本、約 $2.8 と 90 秒の無駄）。planner が依頼文に「**観点レビュアーを background で起動しないでください**」と書いて自力でやり直していたのが発見の端緒。`agents/reviewer-leader.md` に「返ってくるまで待つ」を明記し、#105 ではリーダー → 観点の順に起動して 1 回で終わった
    - **しかし支配項は親のまま動いていない。** #104 の planner 1 ラウンドは 33 分・$25.23・cache read 27.7M。#105 は 15 分で打ち切って planner だけで $15.15・cache read 14.3M と、**単位時間あたりのペースが同じ**。サブエージェント側は #105 で合計 $4.88（全体の 24%）で、#104 の 30% から構造は変わっていない。**観点を減らしモデルを下げても、1 ラウンドの費用は下がらない**
    - **残る未測定**: grilling をラウンドまたぎで継続する変更（`SendMessage`。`2eccea4`）が効くか。#105 は 1 ラウンド目で打ち切ったため判定できていない。**判別は「`conversations/*-grilling.md` が 2 本以上あるのに、grilling の transcript が 2 本のまま」**（作り直しなら 2 役 × ラウンド数になる。#104 は記録 2 本に対し transcript 4 本だった）
  - **ローカルの測定が水増しされていた（2026-09-18 に発見、`1e638fe` で対処）。** planner の文脈に開発者個人の MCP サーバーのツール定義が 120 種類（Notion・Slack・Drive・chrome-devtools 等）載っていた。同じ最小プロンプトでの総入力トークンは **69,269 → 29,447**（`--strict-mcp-config`）。**CI のランナーにはユーザーの MCP 設定が無いので、それまでの $119 / $50 / $20 は CI の値ではなかった。** 同じ 15 分の窓での比較（#105 あり / #106 無し）: 親の初回キャッシュ作成 96,894 → 36,496、1 回あたり cache read 217,258 → 65,604、親の費用 $15.15 → $7.08。**35% 多く仕事をして費用は半分**になった
  - **`CLAUDE.md` と `.claude/skills/` は全エージェントに載り、サブエージェントにも同一内容が引き継がれる（実測）。** トランスクリプトの `instructions` attachment が親とサブで**まったく同じ 20,188 文字**で、中身は配布先の `CLAUDE.md`。内訳は `CLAUDE.md` 5,936 トークン + `.claude/skills/` 7,902 トークンで、compass-wiki は `fetch` / `ingest` / `query` の 3 skill（計 24,730 バイト）を git 管理下に持つ。**MCP は親にしか載らないがこの 2 つは引き継がれる**ので、サブエージェントを増やす設計ほど効く（#104 のようにサブ 10 本なら 20,188 文字がのべ 11 回）。**切る手段は無い** — `--bare` は CLAUDE.md 自動読み込みを止めるが OAuth を読まなくなり（実測で「Not logged in」）、CI の WIF でも成立しない。`--setting-sources` では止まらない（`project` 指定でも同じ 29,448 だった＝配布先自身の skills だから）。**local↔CI では一致するので測定は壊れていないが、契約が定める `.agent/conventions.md`（2,160 バイト）の 20 倍の入力が契約外の経路で入っている**
  - **点検項目 10 個目（`10b58bc`）はまだ一度も試されていない。** #104 / #105 / #106 はいずれも planner の途中で打ち切ったため `reviews/` が空で、判定が出ていない。差し戻しの実例は #101 の 2 回だけで、どちらも修正前
  - **差し戻しの再発を追った結果（#106、2026-09-18）。検出は効いていて、壊れていたのは「直し方」だった。** 点検項目 10 個目（`10b58bc`）の前半「根拠が計画の中に挙がっている」は効き、planner は自分で `sources` を 7 本 → 9 本に増やした（#101 では本数を固定したまま記述を削る方向へ逃げ、2 ラウンド続けて転んでいた）。**しかし後半「根拠の一覧を本数で固定しない」が項目の末尾に埋もれ、誰も拾えていなかった。** レビューチームは機能面・非機能面の**両方から重複で**本数固定の矛盾を指摘していたが、添えた直し方が「本数を合わせて直す」という症状への対処だったため、planner はそのとおり増やし、plan-reviewer に同じ形で差し戻された。対処（`6bb3982`）: (1) 「根拠の本数を書かない」を `## 禁止` に格上げ (2) 点検の一覧で独立した項目に分割（10 → 11） (3) **リーダーに「直し方を添えるなら、同じ問題が再発しない形を選ぶ」**を足した。**チームの価値は検出にあり、処方にはない**ことが分かったので、処方の質を上げる指示を定義側に置いた
  - **したがって次に効かせるなら親側**: `max_turns: 150`、`prompts/planner.md` の分量（433 行）、毎ターン再送される履歴。サブエージェントの本数やモデルではない
  - 未測定: developer 以降をローカルで実物のエージェントに通していない（中断したため）。developer はチーム持ちで `max_turns: 300` なので、計画より重い可能性がある — Q-6、`e330a63`、`b5d23d1`
- [x] V-19: **plugin はヘッドレス（`claude -p`）で効く。4 点を実測した（2026-09-16、ローカル）。** 検証用の最小 plugin（`agents/` 1 つ + `hooks/hooks.json`）を作り、`--plugin-dir <path>` を付けて `claude -p` を走らせた結果:
  - **`--plugin-dir` はヘッドレスで読まれる**（プロジェクト外の一時ディレクトリでも可）
  - **plugin が配る agent 定義を `Task` から名前で呼べる。** 定義の本文がそのエージェントの指示として効いた（指定した 1 行だけが返った）
...
- [ ] V-18: **`num_turns` と `--max-turns` が数えているものを確かめる。** 2026-09-14 の creal/compass issue #276（live）で、**plan-reviewer は `--max-turns 25` で走りながら `num_turns: 40` を報告して `subtype: success` で終わった**（3 ラウンドとも 38〜40）。一方 developer は `--max-turns 60` に対し `num_turns: 61` で `error_max_turns` になっている。つまり結果 JSON の `num_turns` は `--max-turns` が強制している counter とは別物の可能性が高く、**役割ごとに上限値の意味が揃っていない**。上限を決める根拠に直結する（実測が「余裕あり」に見えていても実際は違う、またはその逆）。同じ run の実測は次のとおり:
  ...
  |---|---|---|---|---|
  | planner 1 | 34 | 33.9 分 | 60 秒 | $9.30 |
  | plan-reviewer 1 | 40 | 7.1 分 | 11 秒 | $2.18 |
  | planner 2 | 23 | 14.2 分 | 37 秒 | $4.21 |
  | plan-reviewer 2 | 39 | 3.8 分 | 6 秒 | $1.63 |
  | planner 3 | 18 | 11.5 分 | 38 秒 | $3.17 |
  | plan-reviewer 3 | 38 | 4.5 分 | 7 秒 | $1.87 |
  | developer | 61（上限超過） | 6.8 分 | 7 秒 | $3.86 |

  読み取れること: **planner はターンが安く時間が高い**（`Task` を 1 回呼ぶと親のターンは 1 しか進まないのに、裏でサブエージェントが 1 セッション走る）。**developer は逆で、1 ファイルの編集も 1 回のテスト実行も 1 ターン**を消費する。planner の上限は時間側（90 分）、developer は回数側（300 ターン）で決まる
...
- [x] A-11: **解消（2026-09-10。K-30 / K-31）。** `scripts/release.sh` がタグを打つときに、中央の自己参照（composite action・reusable workflow・中央の checkout の `ref:`）をそのバージョンに書き換えたコミットを作る。配布物 `install/agent-pipeline.yml` も正確なバージョンを指すので、**配布先は 1 つのタグから自己完結したバージョン（workflow + `dist/cli.js` + prompts）を引ける**。実機で確認済み（`role_prompt` が `_actions/.../v1/prompts/planner.md` を指した / I-13）
...
- [ ] A-17: 設計書 §4.1 の `run.yml` を廃止し、`agent-dispatch.yml` の `run` job + composite 構成に置き換える。構成案 §9 の反映 — 構成案 §9
...
- [x] A-60: **判断の記録に `status` を足した（2026-09-09）。採択 / 未処理 / 取り下げ。**
  - `adopted`（計画や実装に反映した）/ `open`（**未処理**。根拠が足りず決められなかった。本文に選択肢と推奨を残し、`plan.md` の「前提」にも未確認として書く）/ `dropped`（取り下げ。問いが成立しなくなった、または試して捨てた案。**なぜ取り下げたかを本文に書く** — 同じ道を次の実行が試さないため）
...
- [x] A-62: **計画の点検リストを作った（2026-09-10）。planner が自己点検し、plan-reviewer が同じ一覧で見る。**
  - **`- [ ]` の形の 7 項目**（プロンプトの中の一覧。`plan.md` に書き写させない）: (1) **issue の完了条件がすべて `AC` に対応**（実機 #25 で差し戻された形） (2) ストーリーと `AC` の対応表が両方向で埋まっている (3) **INVEST**（I: 他を待たずに出せる / N: ストーリーにファイル名やコマンドを書かない / V: 受け手の利益 / E: 見積もれる / S: 1 PR に収まる。超えるなら正常系・CRUD・アクターで切る / T: 確かめる方法がある） (4) 触ってはいけない領域が変更対象に無い (5) 規模判定の数が変更対象と一致 (6) `automated` の `command` が実際に走る形（走らせられないなら `manual` + `evidence` の引き方を条件に書く） (7) 前提が推測で埋まっていない（未確認は `status: open` の記録に選択肢と推奨を残す）
  - **機械検査は入れない**（`validate` に対応表の突き合わせを足す案は保留）。**代わりに一覧を 2 つのプロンプトに置き、`<!-- checklist:start -->` の間が一致していることをテストで固定**した（片方だけ直ると点検が食い違うため）
  - **「確かめた」と書かせない。** 自己点検の目的は直すこと（または未確認として前提に出すこと）で、チェック済みの宣言を成果物に増やさない
- [x] A-63: **受け入れ条件の書き方を整えた（2026-09-10）。UnitTest でカバーする条件の表現。**
  - **`description` は命題、`command` はその証明手段**（「`Foo` は期限切れのとき nil を返す」＋「`bin/rails test test/models/foo_test.rb -n /expired/`」）。スイート全体だけを書くと、緑でもその条件を検証したとは言えない
...
- [x] A-65: **`.agent/**` の変更をハーネスが弾く / 配布物から `id-token: write` を落とす（2026-09-11。`creal/compass` のレビュー指摘）。**
  ...
  - **使っていない権限は配布先に与えない。** `install/agent-pipeline.yml` の `id-token: write` を落とした（WIF に切り替えるときは、どのみち caller の `uses:` のバージョンを上げるので、そのついでに足せる）。テストで「配布先のラッパーは `id-token` を宣言しない」を固定
  - 指摘のうち「中央リポジトリを `creal` org へ移す」は**今回は見送り**（配布先が増えたら再検討。属人性の論点は残っている）
- [x] A-66: **grilling の 2 役を廃し、計画を詰める問いにはレビューチームが答える（2026-09-24。K-34）。**
  - **planner は問いを立てるが、答えは採らない。** 問いは frontier ごとにまとめ、推奨答を添えて `reviewer-leader` に 1 ラウンド 1 回で渡す。リーダーは問いごとに観点へ振り分け（語の解釈・要件の範囲は機能面、実行可能性・後から変えにくい選択は非機能面）、観点どうしで食い違えば**どちらも選ばず「割れた」**として返す
  - 答える規則（根拠は issue・規約・実ファイルの 3 つだけ、無ければ「未確認」、推奨に引きずられない）は `grilling-answerer` から `reviewer-functional` / `reviewer-nonfunctional` に移した。**やり取りの記録の形式は `reviewer-leader` の定義に持たせた**（K-32。`<slug>` は `leader-consult`）
...
- [x] A-68: **planner は観点の 2 人を直接呼ぶ。観点は計画の中身を見る。記録は planner がラウンドごとに書く（2026-09-25）。**
  ...
  - **測った結果（2026-09-25、compass-wiki issue #109。#101〜#108 と同じ本文・同じ土台 `fc82f09` の初回の計画）**:
    - **待つ・記録するは直った。** 観点の答えは全ラウンドで両方そろい、判断の記録 10 件はラウンドごとに書かれた。点検の機能面の指摘 3 件は、どれも新しく置いた「計画を見るとき」の項目に当たった。#108 で 5 回中 3 回差し戻された `counterparty.md:38` との矛盾は、計画を詰める問いの段階で避けられた
    - **差し戻しは防げなかった**（`request_changes`、理由 2 つ）。(1) 権限の比較で取引先一覧を落とした — 非機能面の答えが誤った前提のまま「割れた」で open に入った (2) issue の完了条件「同じ構成・同じ粒度」を確かめる AC が無い — 今回チームから外した領域で、著者の自己点検が見逃した（#104 ではチームがまさにこれを拾っていた）
    - **費用は下がらなかった。** planner 本体 $8.30（83 ターン、cache read 10.6M）+ 観点 $1.23 + plan-reviewer $1.49 = $11.02。サブエージェントは $2.6〜5.2 → $1.23 と下がったが、事実の読み込みを本体に移したため本体の `Read` + `Grep` が 71 回（#107 は 39 回）になった
    - planner がラウンド 3 で問いを片方の観点にだけ投げ、合わせ方の表に無い判断を足した
- [x] A-69: **事実の読み込みを `researcher`（Haiku）に任せる / issue の完了条件の網羅を機能面の点検に戻す / planner のプロンプトを削る（2026-09-25）。**
  - **読み込みは短命なサブエージェントで。** A-68 の結果、本体が読んだものは本体の長い履歴に残って毎ターン再送された。`agents/researcher.md`（`claude-haiku-4-5-20251001`、Read / Glob / Grep）が事実をパスと行番号で返し、**planner はリポジトリのファイルを開かない**。判断はしない役なので、Haiku で足りると見た。誤りは観点（Sonnet）と plan-reviewer が実ファイルで拾う
  - **「issue の完了条件が受け入れ条件で確かめられるか」だけを機能面の「計画を見るとき」に戻した。** 形式（コマンドの形・`--strict`・grep の書き方）は戻さない。A-68 で外したら、著者の自己点検が同じ項目を持っていても見逃した
...
- [x] A-72: **`plan-format` と `plan-checklist` を、skill `plan-writing` の reference にした（2026-09-25）。**
  ...
  - **実装タスクで最後まで通した（2026-09-25、compass-wiki issue #111 = #13 の本文「`bin/validate --help` が使い方を表示しない」、ローカル、`9e63452`）。** `bootstrap` → planner → plan-reviewer（差し戻し）→ planner → plan-reviewer（承認）→ 承認 → developer → dev-reviewer（承認）→ completion → **`done`**。55 分・約 $18.7
    - **skill はすべての役割で段の直前に呼ばれた。** planner は各ラウンドで `plan-grilling`・`shared-team`・`shared-journal`・`plan-writing`（`format.md` と `checklist.md` の両方を読んだ）。plan-reviewer は `plan-writing`（`checklist.md` だけを読んだ）と `shared-verdict`。developer は `shared-team`・`shared-journal`・`shared-evidence`。dev-reviewer は `shared-evidence`・`shared-verdict`。completion は `shared-evidence`
    - **developer はリーダー無しで観点の 2 人を直接呼び、答えを受け取れた**（A-71 の狙い）。やり取りの記録（`team-consult` / `team-review`）も developer 自身が書いた。観点の答えが割れた問い（`allow_abbrev` を切るか）は、developer が理由と捨てた案を付けて自分で決めた（developer は決めて記録する、という設計どおり）
  ...
  - **CI で回した（2026-09-25、compass-wiki issue #36 = #13 の本文、`v1.0.3` を `712d70c` で上書き。上書き前の `v1.0.3` は `055630e`）。** 引数は正しく渡った（`--plugin-dir /home/runner/work/_actions/satoshiarai-rgb/agent-pipeline/v1.0.3`、`--tools …,Skill`）。`modelUsage` に Haiku と Sonnet が出ており、やり取りの記録・判断の記録・`plan-writing` の形式どおりの `plan.md` まで書かれた — **skill と reference は CI でも効いた**
    ...
    - **手当ての後、CI で `done` まで通った**（`v1.0.3` = `5010198` の上書き、PR #37 は ready for review）。経路は planner（やり直し）→ plan-reviewer（差し戻し: `import sys` の扱いと、パスとフラグの交互配置が argparse で通らない — ローカル #111 と同じ穴をゲートが拾った）→ planner → plan-reviewer（承認）→ `/agent approve` → developer → dev-reviewer（承認）→ completion。**失敗した 1 回目を含めて約 $33.2**（失敗 $12.6 / planner やり直し $2.5 / plan-reviewer $1.5 / planner 2 回目 $8.8 / plan-reviewer $1.6 / developer $3.6 / dev-reviewer $1.6 / completion $1.1）
    - **CI の planner はローカルより重い。** 2 回目は 105 ターン・16.6 分・$8.8 で、ローカル #111 の 2 回目（$3.2）の約 3 倍。1 回目の 151 ターンと合わせて、CI の planner が何にターンを使っているかは測る必要がある（V-18）
    - **CI の developer は完了時の点検の記録を残さなかった。** やり取りの記録は相談（`team-consult`）の 1 本だけで、`shared-team` が定める完了時の点検（`team-review`）が無い。点検を飛ばしたのか、記録しなかったのかは transcript が無いので分からない（ローカル #111 では点検して記録も書いた）
    - **実行記録で原因が分かった（2026-09-25、compass-wiki #38 の planner、`num_turns` 109・15 分・$8.78）。** 本体は完了時の点検を観点の 2 人に `SendMessage` で送り（「Both are still busy… Waiting.」）、**答えが届くまでの約 40 回を、無関係なディレクトリの `Glob`（`work/amdb-compass/*`、`raw/**`、`docs/*/*.md`、`.claude/*.json` など）で空打ちした**。最後は「機能面がまだ返らないので、いまの状態で記録する」と見切ってから、届いた答えで書き直している。`SendMessage` で継続した相手の答えは後から届き、ヘッドレスの親には待つ手段が無い。#36 の 151 ターンの `error_max_turns` も、#36 の developer が完了時の点検の記録を残さなかったのも、同じ形と見ている → A-73
    - **実行記録を artifact に上げるようにした（2026-09-25）。** `agent-dispatch.yml` の `agent` の直後に `actions/upload-artifact@v7`（`always()`、名前は `execution-<agent>-<run_id>-<attempt>`、保持 14 日）。リポジトリには push しない。中身は transcript なので、配布先のリポジトリを読める人は読める
    - **issue の前提の半分が誤りだった**: CI のランナーには `python3`・`pyyaml`・`jsonschema` があり、`python3 bin/validate` は動く（`uv` が無いのでシェバン経由だけが動かない）。developer がこれを問いにし、観点が割れたので `evidence` は issue の指定どおりコードの行番号にした。実測は判断の記録と `staged/pr-body.md` に残した
- [x] A-73: **サブエージェントは毎回、前景の `Task` で呼ぶ。`SendMessage` での継続をやめ、どのプロファイルにも渡さない（2026-09-25）。**
  - 前景の `Task`（`run_in_background: false`）は答えが返るまで親を止めるので、空打ちが起きない。代わりに観点と researcher は毎回作り直され、前の回を知らない（前の回で分かったことは依頼文に書く）。**読み直しの費用（Sonnet の観点で 1 回 $0.3〜0.5）のほうが、空打ち 40 回で本体の長い履歴を再送する費用（Opus で $2 前後）より安く、答えも取りこぼさない**
...
- [ ] R-2: **配布先 2 つ目 = `creal/compass`（Rails 8.1 / MySQL）。導入 PR は出した（2026-09-10。creal/compass#263）。**
  ...
  - **中央に 1 つだけ足りなかったもの → A-64**: 準備をフェーズで絞るための `AGENT_NAME`。読むだけの planner / plan-reviewer でイメージをビルドすると、5 フェーズすべてが数分を払う
  - `setup.sh` は**失敗してはいけない**（非ゼロで終わるとそのフェーズが `agent_failed`）。compass の `setup.sh` は各段で失敗しても警告を出して `exit 0` し、**規約側で「準備に失敗した run では `manual` に落として PR の CI を引く」**と指示した
  - **テストはエージェントが実際に走らせる**（2026-09-10 に (A) で確定）。`compose.yaml`（web + mysql:8.4）に乗り、`setup.sh` が `up -d --wait db` → `up -d --build web` → `db:test:prepare` まで進め、**コンテナを起こしたまま `docker compose exec` で叩く**（`run` だとテスト 1 回ごとに 3〜8 秒余分にかかる）。`.:/app` をマウントしているので編集はビルドし直さずに反映される。**Dockerfile が `BUNDLE_WITHOUT` を使っていないので test グループの gem がイメージに入る**ことも確認した（本番用イメージだと test gem が抜けていて詰む定番の落とし穴）
  - 反復あたりの差は小さい（`exec` の overhead は 1 秒未満で Rails の起動時間に埋もれる）。差が出るのは**準備の 4〜8 分**（`bundle install` + `assets:precompile`。run ごとに新しいランナーなのでキャッシュは効かない）。**重ければネイティブ（apt + Ruby prebuilt + `docker run mysql`）に切り替える**か、中央に「キャッシュのパスとキーを入力で受ける」を足す（`actions/cache` はアクションなので配布先からは足せない。ただし `hashFiles` は caller の `with:` では効かないのでキーは静的になる）
  - **マージ前に人がやること**: (1) GitHub App を `creal/compass` にインストール（Contents / Issues / Pull requests の Read & Write。Workflows は与えない） (2) Secrets 2 つ（`AGENT_APP_CLIENT_ID` / `AGENT_APP_PRIVATE_KEY`。`CLAUDE_CODE_OAUTH_TOKEN` は既にある）。**`agent:go` ラベルは作成済み**
  - マージ後: `AGENT_DRY_RUN=true` で dry-run 一巡 → `false` に戻す。**そのとき `CLAUDE.md` の「組織アカウントには触らない」（K-1 / K-6）を書き換える**（R-3 の前倒し）
...
- [x] I-6: **ダミーバージョン `check-agent-dispatch.yml` で状態機械を実機で一巡させた（2026-09-04）**。`planning` から `done` まで、人間の承認を挟んで 6 本の push ランで完走。`[skip ci]` による二重起動の抑止、`awaiting_human` での停止、`done` での連鎖停止をすべて確認
...
- [x] A-19: **配布先の `.agent/config.json` を既定に重ねる（2026-09-07）。** 規則は `src/utils/mergeSettings.ts` の 4 つ: 書いたキーだけを深く重ねる / `null` は継承 / **既定に無いキーと型違いはエラー**（誤字を黙って無視しない。回数と分数は 1 以上の整数に限る） / 上書きできるのは `OVERRIDABLE` の表にあるキー（models・limits・tool_profiles・agents・approvers。**`labels` は A-55 で上書き不可にした**）だけで、**フェーズの遷移と `pipeline_version` は中央のもの**。読み込みは `src/file/configFile.ts`（`readConfig`）で、**受け付けられないときは一部だけ適用せず全体を捨てる**（どの設定で動いたのか分からなくなるため）。マージ後にしか分からない整合性（`agents.*.tools` が `tool_profiles` にあるか）は `CONSISTENCY` の表で見る
...
- [x] I-7: **完了。** `agent-bootstrap.yml` / `approve.yml` / `agent-comment.yml` として実装済み。以前の記述: `agent-bootstrap.yml` と `approve.yml` を reusable workflow として作成（Step B-4）。C-1 / C-2 で issue のラベルと `/approve` コメントを入口にするときに、認可（`author_association`）と PR 側コメントの除外（A-13）を足す
- [x] I-7b: **完了。** 同上。以前の記述: `agent-bootstrap.yml`（A-12）と `approve.yml`（A-13）
- [x] I-9d: **`agent-dispatch.yml` の本番経路**（2026-09-05）。`compose` → `base-action@v1.0.215` → `validate` → `finish` を繋いだ。`dry_run: true` のダミー経路も同じ tail（validate → finish）を通るようにしたので、**ダミーでも契約違反は blocked になる**（配線の検証にトークンが不要なまま、validate の経路が実際に走る）
  - `agent-bootstrap.yml` が issue 本文を `agent-work/issue-<n>/issue.md` に保存する（`gh issue view --json body -q .body`）。これが無いと planner に issue が渡らなかった
...
- [x] I-3: **完了。** 認可は `agent-bootstrap.yml` の権限確認と `approvers`、scaffold は `agent-bootstrap.yml`、スキーマ検証は `validate`。以前の記述: `src/authorize.ts`、`src/scaffold.ts`、スキーマ検証（A-20）
...
- [x] A-9: **完了。** composite ではなくリポジトリ root の `action.yml` + `dist/cli.js` にしたため `$PIPELINE` 依存が消えた。以前の記述: `$PIPELINE` への依存を除去する。`agent-dispatch.yml` の `mark started`、`approve.yml` の authorize / transition、`agent-bootstrap.yml` の scaffold は composite ではない inline `run` から中央のスクリプトを呼んでいるが、中央リポジトリは checkout されていない。`state-start` / `authorize` / `scaffold` を composite action として切り出し、スクリプト呼び出しを composite 内に閉じる — 構成案 §4.2 / §4.3 / §4.1
...
- [x] A-12: **完了。** `agent-bootstrap.yml` は `--body-file` を使っている。以前の記述: `agent-bootstrap.yml` の `gh pr create --body "...\n\n..."` を `--body-file` に変更する。bash のダブルクォート内では `\n` がリテラルとして入る — 構成案 §4.1
- [x] A-13: **完了。** PR 側のコメントだけを受ける形で実装（`agent-comment.yml` + 配布先の `if`）。run のディレクトリは `headRefName` から導出する。以前の記述: PR 側のコメントだけを受け付ける（K-12）。 配布先の `if` を `github.event.issue.pull_request != null` にし、中央の `approve.yml` は PR 番号から `headRefName` を引いて run のディレクトリを導出する（`agent-work/${branch#claude/}`）。issue 側のコメントは無視する。以前の記述: `issue_comment` は PR でも発火し、そのとき `github.event.issue.number` は PR 番号になるため、`approve.yml` の `ref: claude/issue-<number>` が存在しないブランチを引いてジョブが落ちる。PR 経由の `/approve` を受け付けるなら PR 番号 → issue の逆引きを実装する（どちらを採るか要決定、Q-1） — 構成案 §3 / §4.3
...
- [x] A-39: **完了。** `agent-comment.yml` が `/agent request-changes <理由>` を受け、本文を `reviews/plan-NN.md` に残す。以前の記述: `/request-changes` の入口を配布先 `agent.yml` と中央 `approve.yml` に追加する。 設計書 §3.1 は `awaiting_human` からの出口を `/approve` の 1 本しか定義していなかった。人間が計画に変更を求める経路が無く、`phase` を手で戻しても planner は理由を知らないまま同じ計画を再生成する。実装済みの `requestChangesRun`（コメント本文を `reviews/plan-NN.md` に `verdict: request_changes` / `reviewer: human:<association>` として残し `planning` へ戻す）をワークフローから呼ぶ。設計書 §3.1 / §3.2 / §6.5 とラベル射影に反映する — 実装は `src/index.ts`
- [x] A-43: **完了。** `agent-bootstrap.yml` が draft PR を開き、本文にコマンド表を書く。以前の記述: bootstrap で draft PR を作る。 K-12 により PR 側のコメントを入口にするため、PR が無いと承認できない。本文に `Closes #<n>`、`plan.md` へのリンク、使えるコマンド（`/agent approve` / `/agent request-changes <理由>`）と「承認は PR 側で行う」旨を書く。`--body-file` を使う（A-12）— 設計書 §6.2、K-12、K-13
...
- [x] I-12: **`install/` 一式（2026-09-07）。** 配布先に置くファイルの原本を `install/` に集めた。`agent.yml` → `.github/workflows/agent-pipeline.yml`、`conventions.md` → `.agent/conventions.md`、`setup.sh` → `.agent/setup.sh`、`issue-template.yml` → `.github/ISSUE_TEMPLATE/agent-task.yml`。`install/install.sh` が 4 つをまとめて置く（既存は上書きせず飛ばす / `--force` で上書き、`AGENT_PIPELINE_REF` でバージョンを選べる、置いたあとの手順を最後に出す）。`install/README.md` が置き場所の対応表と前提（下の A-46）とコピー手順を持つ。テストは一時ディレクトリで `install.sh` を実際に走らせる（`bash -n` では見つからない失敗を捕まえる。実際に踏んだ: `$var` の直後に全角文字を書くと bash が変数名の一部として読み、`set -u` で落ちる）。`config.json` の雛形は設定マージ（A-19）と同じ日に足した — 効かない設定ファイルの雛形を先に置かないため、順序をこうした
  - [x] A-51: **配布先ワークフローの正を `install/agent-pipeline.yml` にした。** `work/verify/check-agent-dispatch.yml` を削除し、`docs/installation.md` は inline の YAML をやめて参照（+ イベントと権限の要約）に変えた。`scripts/__tests__/workflows.test.ts` が `install/agent-pipeline.yml` を中央のワークフローと一緒に検査する（呼び出し側の権限が中央を満たすか / 式に算術が無いか / 参照 ref が揃っているか）。検証用の手動起動（`workflow_dispatch` で scenario を選ぶ）は Step B-4〜C-1 用だったので落とした
