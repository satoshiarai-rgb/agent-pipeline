...
- [ ] A-53: **reducer/action への移行（K-26 / K-27）。段取りは 1 → 6。1〜4 はタグ `v1`（I-13）と 2 つ目の配布先（R-2）より前に置く**（`state.json` の役割とイベントのファイル形式が変わるため、配布後だと移行が要る）
  - [ ] 1: **action の FSA 化とガード表。** `finish` / `approve` / `request-changes` / `retry` / `block` の判断を `combineReducers({ info, app })` の reducer に畳み、拒否理由の語彙を `guards.ts` に集める。`redux` を版固定で追加。**受け入れ条件: `blocked_reason` の文字列を 1 文字も変えない**（`explain` の `ADVICE`・`docs/troubleshooting.md` の表・プロンプトが文字列に依存する）。既存テストが通ること
