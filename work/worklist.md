...
| K-30 | **配布先は正確な版（`@v1.0.3` のようなパッチまで含むタグ）を参照する。タグの中の中央の自己参照も同じ版にする**（2026-09-10 決定。K-29 の「移動タグ `@v1` を参照する」を置き換え） | 移動タグだと、パッチを入れた瞬間に配布先の挙動が変わる。**同じ入力で同じ動きが再現できること**を優先し、上げるときは配布先の `uses:` を書き換えて PR にする（4 行）。`v1` は「最新の v1 系」を指す目印として残すが、既定では参照しない。`scripts/release.sh` がリリースごとに (1) main の `install/agent-pipeline.yml` の参照をその版にして push、(2) タグの中の中央の自己参照（composite action・reusable workflow・中央の checkout の `ref:`）もその版に書き換える。**(2) が無いと、配布先が固定してもハーネス本体（`dist/cli.js`）が別の版から来る**（A-11）。テストは「配布先は `v<major>.<minor>.<patch>`、中央は `main`」を固定する |
| K-31 | **版を上げるのは配布先の作業（`uses:` の 4 行）。自動化は Dependabot に任せる**（2026-09-10 決定） | 正確な版を参照する（K-30）ので、上げないと修正が届かない。中央から配布先へ PR を出すボットは作らない（**既存の仕組みに乗る方が保守が要らない**）。`install/dependabot.yml` を配布物に足し、`github-actions` の ecosystem を置く（既に `.github/dependabot.yml` があるリポジトリでは `install.sh` が skip するので、項目を自分で足す旨を案内に書いた）。**`install.sh --force` は上げる手段ではない**（`.agent/` の雛形まで戻る）。`pipeline_version` が上がる版に切り替えるときは、**進行中の run が無いタイミングで上げる**（噛み合わないと `pipeline_version_mismatch` で止まる。止まった issue は立て直す / K-14） |
...
- [x] Q-3: **決定（2026-09-10。K-30）。許容しない — 配布先は正確な版（`@v1.0.3`）を参照する。** 移動タグだとパッチを入れた瞬間に配布先の挙動が変わり、進行中の run の途中でプロンプトが入れ替わる。版を上げるのは配布先の作業（`uses:` の 4 行）で、自動化は Dependabot に任せる（K-31）
...
- [x] A-11: **解消（2026-09-10。K-30 / K-31）。** `scripts/release.sh` がタグを打つときに、中央の自己参照（composite action・reusable workflow・中央の checkout の `ref:`）をその版に書き換えたコミットを作る。配布物 `install/agent-pipeline.yml` も正確な版を指すので、**配布先は 1 つのタグから自己完結した版（workflow + `dist/cli.js` + prompts）を引ける**。実機で確認済み（`role_prompt` が `_actions/.../v1/prompts/planner.md` を指した / I-13）
...
- [x] I-6: **ダミー版 `check-agent-dispatch.yml` で状態機械を実機で一巡させた（2026-09-04）**。`planning` から `done` まで、人間の承認を挟んで 6 本の push ランで完走。`[skip ci]` による二重起動の抑止、`awaiting_human` での停止、`done` での連鎖停止をすべて確認
