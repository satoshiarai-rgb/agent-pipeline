...
| K-32 | **フェーズの中のチーム（レビューチーム・grilling の 2 役）は agent 定義として中央が配り、配布先の上書きは想定しない**（2026-09-16 決定） | 振る舞いの指示を親のプロンプトからバケツリレーさせると薄まる（実測: 「リーダーには要約させる」が届かず、やり取りの記録が 7.6KB → 19.6KB に増えた）。定義ファイルに直接書けばリレーが消え、親が渡すのは「何を見てほしいか」だけになる。**上書きの経路は増やさない** — フェーズのプロンプト（`.agent/prompts/<agent>.md`）は配布先が差し替えられるが、チームの定義は中央のものを使う。`.claude/agents/` に置けば配布先が上書きできてしまうが、`.claude/**` は人間しか置けない領域（K-19）で、差し替えの入口が 2 系統に割れるため採らない |
