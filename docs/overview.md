...
- **品質のガードが構造に入っています。** 作る役と批判する役を別のエージェントに分け、
  レビューする側には**成果物だけ**を渡します（作った側のセッションログや思考過程は渡しません。
  計画のレビューには元の issue も渡り、実装のレビューには計画・受け入れ条件・判断の記録と
  git の差分が渡ります）。受け入れ条件を満たさない限り完了になりません
...
| 4 | 実装 | 計画、受け入れ条件、計画のレビュー、実装のレビュー | **コード**、`acceptance.json` の結果更新、`journal/*.md`（計画に無い判断） | 差分があり、形式を満たす |
...
├── journal/            進行中の判断（判断 1 つにつき 1 ファイル。種類と後戻りの容易さ付き）
├── decision-records/   そのうち後戻りが困難なものだけ（ADR の候補。ハーネスが寄せる）
├── reviews/            plan-01.md, plan-02.md, dev-01.md ...（各レビューの判定と指摘）
├── completion.md       完了報告（やったこと・条件の結果・確認してほしいこと）
├── events/             **状態の正。** 起きたことの記録（実行の開始と終了、人間の介入）
└── state.json          現在の状態
```
...
3. draft PR が開きます。**計画ができると PR にコメントが付き、読むべきファイル
   （`plan.md`・`acceptance.json`・計画レビュー）へのリンクが並びます**
4. その PR にコメントする
   - `/agent approve` — 計画を承認して実装に進む
   - `/agent request-changes <理由>` — 計画を差し戻す。理由がレビューとして残り、次の計画に反映されます
5. 実装とレビューが終わると draft が外れ、**完了報告・判断の記録・全レビューへのリンクが
   コメントで届きます**。あとは通常の PR レビューです

**あなたに手番が回るときは必ず PR にコメントが来ます**（計画の承認待ち、実装の完了、
途中で止まったとき）。止まったときは理由と次の手順が書かれているので、原因を直して
`/agent retry` とコメントすれば同じフェーズをやり直します。
