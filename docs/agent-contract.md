...
| 出力（任意） | `conversations/<run_id>-<attempt>-<agent>-<NN>-<slug>.md` — **エージェント同士のやり取りの生ログ**（1 往復につき 1 ファイル。`<slug>` が相手を表す: `team-consult`（相談。planner なら計画を詰める問い）/ `team-review`（完了時の点検）。書くのは相談した planner / developer 自身）。**ハーネスは名前を渡すだけで、中身は読まないし検査もしない**（遷移に関わらない保管） |
...
- `status` は判断の行き先。**決めきれなかった問いも記録として残す**（消さない）
  ...
  | `dropped` | 取り下げた。問い自体が成立しなくなった、または試して捨てた案。**なぜ取り下げたかを本文に書く**（同じ道を次の実行が試さないため） |
...
| ツール | `--tools` でエージェントごとに絞る。planner と plan-reviewer に `Bash` は渡さない（A-30）。**`Task` を持つのは planner と developer だけ**。planner は事実の読み込みを `researcher`（Haiku）に任せ（A-69）、観点の 2 人（`reviewer-functional` / `reviewer-nonfunctional`）を直接呼ぶ（計画を詰める問いと完了時の点検 / A-66・A-68）。developer も観点の 2 人を直接呼ぶ（相談と完了時の点検 / A-71）。レビュアーには渡さない。**`SendMessage` はどのエージェントにも渡さない** — サブエージェントは毎回、前景の `Task` で呼ぶ（継続した相手の答えは後から届き、ヘッドレスでは待つ手段が無いため / A-73）。**`Skill` は全エージェントが持ち**、コアな動作（計画の詰め方・チームの呼び方・判断の記録・evidence・判定の書き方・点検の一覧・成果物の形式）を plugin 同梱の skill（`skills/`）から段の直前に読む（A-70・A-71）。サブエージェントも親と同じ許可の下で動くので、planner のサブエージェントはコマンドを実行できない |
