...
参照先は **`@v1`** です（`install.sh` が置くファイルはそうなっています）。`v1` は
**移動するタグ**で、後方互換な修正を入れると付け替わります。壊れない範囲で自動的に
追随したいので、通常はこれを使ってください。

- 完全に固定したいなら `@v1.0.0` のような**動かないタグ**に書き換えます（そのままでは
  修正が届かないので、自分で上げる運用になります）
