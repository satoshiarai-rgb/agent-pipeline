...
コメントされます**（PR へのコメントは、止まったとき・計画の承認待ち・実装の完了の 3 つの
場面で来ます）。まずそれを読んでください（`state.json` の `blocked_reason` にも同じ理由が
...
100 分、developer は 100 分）を過ぎていれば、`/agent retry` がそのまま受け付けます。** 同じフェーズを最初から
やり直し、実行中の記録は落ちます。
...
1 PR あたりの目安（テストを除いて 20 ファイル、テストを含めて 40 ファイル）を超えると planner が
判断した、という通知です。
...
| `journal/<run_id>-<attempt>-<slug>.md` | planner / developer | 判断の記録。1 つにつき 1 ファイル（`status` が `adopted` / `open` / `dropped`）。**`reversibility: hard` はハーネスが `decision-records/` へ寄せる** |
